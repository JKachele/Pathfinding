...
public class AStarCardinal {
    ...
    public static  void initialize(Environment panel, JFrame frame) {
        AStarCardinal.environment = panel;
        AStarCardinal.frame = frame;
        AStarCardinal.cells = panel.getCells();
        AStarCardinal.startCell = panel.getStartCell();
        AStarCardinal.targetCell = panel.getTargetCell();
    }
    ...
    public static void setOpenCells(ArrayList<Cell> openCells) {
        AStarCardinal.openCells = openCells;
    }
    ...
    public static void setClosedCells(ArrayList<Cell> closedCells) {
        AStarCardinal.closedCells = closedCells;
    }
    //endregion
    /* ***************Methods*************** */
    ...
    private static void getVariables() {
        AStarCardinal.cells = environment.getCells();
        AStarCardinal.startCell = environment.getStartCell();
        AStarCardinal.targetCell = environment.getTargetCell();
    }
    ...
    private static void openSurroundingCells(Cell centerCell) {
        Vector2[] cardinalDirections = {new Vector2(0, -1), new Vector2(1, 0),
                new Vector2(0, 1), new Vector2(-1, 0)};
        for(Vector2 i: cardinalDirections) {
            if(isValidGrid(centerCell.getGridPosition().x + i.x, centerCell.getGridPosition().y + i.y)){
                Cell cell = cells[centerCell.getGridPosition().x + i.x][centerCell.getGridPosition().y + i.y];
                if(cell.getId() == CellID.OBSTACLE || closedCells.contains(cell)){
                    continue;
                }
                int gCost = centerCell.getGCost() + 1;
    ...
    private static void calcHCosts() {
        for(int x=0; x<cells.length; x++){
            for(int y=0; y<cells[x].length; y++) {
                int hCost = Math.abs(x - targetCell.getGridPosition().x) + Math.abs(y - targetCell.getGridPosition().y);
